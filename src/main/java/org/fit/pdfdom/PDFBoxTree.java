...
public abstract class PDFBoxTree extends PDFTextStripper {
        ...
        //rectangle
            ...
            if (!disableGraphics) {
                if (arguments.size() == 4) {
                    ...
                    graphicsPath.add(new PathSegment(p1[0], p1[1], p2[0], p1[1], rect));
                    graphicsPath.add(new PathSegment(p2[0], p1[1], p2[0], p2[1], rect));
                    graphicsPath.add(new PathSegment(p2[0], p2[1], p1[0], p2[1], rect));
                    graphicsPath.add(new PathSegment(p1[0], p2[1], p1[0], p1[1], rect));
                }
            }
        }
    ...
    protected void finishBox() {
        if (textLine.length() > 0) {
            ...
            curstyle.setTp(textMetrics.getTp());
